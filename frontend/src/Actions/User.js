...
export const searchMembers=(name="")=>async(dispatch)=>{
  try {
    dispatch({
      type:'memberSearchRequest'
    })
    const {data}= await axios.get(`/api/v1/member?name=${name}`);
    console.log(data);
    dispatch({
      type:'memberSearchSuccess',
      payload:data.members,
    })
  } catch (error) {
    dispatch({
      type: "memberSearchFailure",
      payload: error.response.data.message,
    });
  }
}
...
export const getAllManagers=()=>async(dispatch)=>{
  try {
    dispatch({
      type:'allManagersRequest'
    })
    const {data}= await axios.get('/api/v1/admin/users');
    dispatch({
      type:'allManagersSuccess',
      payload:data.users,
    })
  } catch (error) {
    dispatch({
      type: "allManagersFailure",
      payload: error.response.data.message,
    });
  }
}